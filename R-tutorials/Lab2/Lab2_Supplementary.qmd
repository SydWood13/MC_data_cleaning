...
format: 
  html:
    ...
    output-file: Lab2_Supplementary_Page.html
  revealjs: 
    incremental: true
    code-tools: true
    code-copy: true
    ...
    width: 1400
    footer: "PSC103A WQ24"
    output-file: Lab2_Supplementary_Slides.html
editor: visual
---
...
-   First take a quick look at the R Environment.

    ![](images/importedCSV_example.png){width="417"}

    -   ::: {style="font-size:70%"}
        This can tell us that this dataset consists of 5 variables and has a total of 232 data records
        :::

-   We can check the behavior of our dataset by using `head()` to print out the fews rows of the dataset. Because this is only a quick check, we don't have to store it as an object.
    ...
    -   Note: by default, R displays 6 rows if you omit the "n" argument. You can check by using either `?head()` or `help(head)` .
        ...
        ```{r, eval=FALSE}
        head(lab2_data_csv) # will return the same output as above  
        ```
    :::
...
::: {.column width="10%"}
<!--# just to create space between columns-->
:::
...
::: columns
::: {.column width="45%"}
A bar chart requires the frequencies or relative frequencies for each category, so we need to **create a new object** that includes the frequencies/relative frequencies of each category. Steps to plot in R:
...
3.  Save your report as **either a HTML, PDF or MS WORD file**
    ...
    ::: nonincremental
    -   ::: {style="font-size: 80%"}
        **Tip: Make sure that all of your code runs correctly before compiling.** For example, your report will not compile if you reference an object that has not been created in your R script.
        :::
    :::
...
\
💡 **Hint**: How many observations are in this dataset? Is that number large or small? What is the variable type? Which display is the most appropriate given the number of observations and data type?
...
```{r, eval=FALSE, echo=FALSE}
hist(activity_data$age, # numeric variable to plot
     main="Histogram of Age", # set title 
     col = 4, # controls the color of the display
     xlab = "Age (in Years)", # label the x-axis 
     freq=F) # set to "T" for the frequency, "F" to plot the density
```
