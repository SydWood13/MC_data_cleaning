...
for (file in Exam4) {
  ...
  # Select the required columns
  exam4 <- exam4 %>% 
    select("SID","View.Count", "Status", 19, 22, 24, 29) ##the 23rd column is appear in many other datasets but 4a
  
  # Rename the columns
  q_names <- c("SID", "views4", "Status", "ov_op", "ov_cv", "confound", "external")
...
```{r}
unit4a<- unit4a %>% select("SID","View.Count", "Status", 19, 22, 24, 29)
...
```{r}
write.csv(unit4a, file = "Unit_4a_cleaned.csv", row.names = F)
```
...
```{r}
write.csv(unit4b, file = "Unit_4b_cleaned.csv", row.names = F)
```
