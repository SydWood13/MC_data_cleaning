...
write.csv(QMistakes, file = "mistakes.csv")
...
time2data <- time2shared %>% mutate(RepExam1 = count1 + count3 - 1) %>%  mutate(RepExam2 = count2 + count3 - 1) %>%  mutate(rep = case_when(!is.na(RepExam1) ~ 1, !is.na(RepExam2)~1, .default = NA )) %>% filter(!is.na(rep)) #%>% select(-question_id2)
...
QMistakes <- QMistakes %>% mutate(time = exam -1)
...
agdata4 <- agdata %>% filter(time == 4) %>% filter(question_id == "COMPCV"| question_id == "Interpret"| question_id == "IVTHREAT" | question_id == "REP")




pMistakes1 <-  QMistakes %>% filter(exam<2) %>% mutate(time = time + 1) %>% mutate(pastMistakes = numMistakes) %>% select(-numMistakes)
