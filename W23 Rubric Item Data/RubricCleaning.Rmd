...
write.csv2(rubric_data, file = "full_rubric_data.csv", sep = ",")
...
#write.csv(QMistakes, file = "test.csv")
...
```{r}
time0data <- mistakes %>% filter(exam == 1)

time1data <- mistakes %>% filter(exam < 3)

time2data <- mistakes %>% filter(exam < 4)

time3data <- mistakes %>% filter(exam < 5)
...
) %>% mutate(rep = case_when(rep>1~1, .default = rep))
...
QRepMistakes1 <- repMistakes1 %>% group_by(student_id, question_id) %>% summarise(
  numRep = sum(rep),
  numTot = sum(tot)# total number of rubric items given across both exams (including repeated items) 
)

QRepMistakes1 <- QRepMistakes1 %>% mutate(uniqueTot = numTot - numRep) # total unique items given on this question 
...
time1data <- mistakes %>% filter(exam < 4)

# time 2 repeated mistakes by Notation 
repMistakes2 <- time1data %>% group_by(student_id, question_id, Notation) %>% summarise(
   = n() -1
)
