...
```{r}
# Package names
packages <- c("ggplot2",  "tidyverse", "apaTables", "colourpicker", "swirl", "dplyr", "gridExtra", "knitr", "lme4", "reshape2", "stargazer",  "did", "plm", "sandwich", "coefplot", "afex", "jtools", "glmmTMB", "psych", "gtsummary", "broom", "optimx")
...
o = sum22$coefficients[1] + sum22$coefficients[2] + sum22$coefficients[8]
...
(sum(nstudent$numRep))/369
(sum(nstudent$numCurrent))/369

nmstudent <- nstudent%>% filter(numRep>0)



(sum(nmstudent$numRep))/314
...
nonzero <- mstudent %>% filter(nRep != 0)
...
nzTime <- nonzero %>% group_by(time) %>% 
  summarise( attempt = "none",
             n = n(), 
             totalRep = sum(nRep),
             mean = mean(nRep, na.rm = T),
             sd = sd(nRep, na.rm = T),
             median = median(nRep, na.rm = T),
             MAD = mad(nRep, na.rm = T)
  )
...
nzdesc <- rbind(nzTime, nzAttempt, mQ)
...
plot <- ggplot(nstudent) +theme(axis.text.x = element_text(face = "bold", color = colors[4], 
                             size =15),
...
plot1 <- plot + geom_histogram(aes(x = pRep), fill = colors[3]) +labs(title = "Distribution of % Mistakes Repeated" ) +xlab("% Repeated") #+ ylab()


ggsave(plot1, file = "repDist.png")
?ggsave
...
rq4eq1 <- "nRep ~ attempt + (1|question_id) +  (1|student_id) "

rq4m <- glmer(rq4eq1, mstudent, family = "poisson")
...
```
INTERPRETATION: Students odds of making a repeated mistake on their 3rd time responding to the prompt, however for the questionsthat were attempt 3 or 4 times prior they were significantly less likely to repeat thier mistakes. 
...
rq4eq1 <- "pRep ~ attempt + (1|question_id) +  (1|student_id) "

rq4m <- lmer(rq4eq1, mstudent)
...
grouping <- data %>% select(student_id, time, opt_in, viewed)

dat <- left_join(mstudent, grouping, by = c("student_id", "time"))
dat$viewed <- as.factor(dat$viewed)

rq4eq2 <- "nRep ~ time + opt_in + viewed + time*opt_in * time*viewed + viewed*opt_in + time*opt_in*viewed + (1|student_id) + (1|question_id) + (1|attempt)"

rq4m2 <- glmer(rq4eq2, dat, family = "poisson",  control = glmerControl(
                           optimizer="bobyqa",
...
#### Interpreatation
Overall students make fewer repeated mistakes over time, but this effect is not different for the different groups nor is it different base on feedback viewing. 
...
rq4eq2 <- "pRep ~ time + opt_in + viewed + time*opt_in * time*viewed + viewed*opt_in + time*opt_in*viewed + (1|student_id) + (1|question_id) + (1|attempt)"

rq4m2 <- lmer(rq4eq2, dat,  control = lmerControl(
                           optimizer="bobyqa",
                                 optCtrl=list(maxfun=2e5)))
