...
## full data 
```{r}
setwd("~/GitHub/MC_data_cleaning/2023 data/new version of exams")

desc <- read.csv(file = "2023_cleaned.csv")
...
score_data <- fulldata %>% select(SID = Real_id, anon_id = student_id, opt_in, p_tot1:p_tot5)
...
data <- data %>% mutate(p_score = case_when(time == "1" ~ p_tot1, time == "2"~p_tot2, time == "3"~p_tot3, time == "4" ~ p_tot4))
...
desc_l <- gather(desc, "Exam", "score", p_tot1, p_tot2, p_tot3, p_tot4, p_tot5)



writtenTable <- desc_l %>% group_by(Exam) %>% summarise(variable = "Performance", n = n(), 
...
rq1data <- data %>% select(p_score, viewed)
...
lin <- "viewed~p_score"
rq1mod1 <- glm(lin, data, family = "binomial")
...
rq1data$linfit <- predict(rq1mod1, data)
...
```
Significant positive linear relationship. Each point increase of written score is a .02 logit increase or a 1.02 percentage point increase in the probability of viewing feedback. 

MODEL FIT:
χ²(1) = 42.06, p = 0.00
Pseudo-R² (Cragg-Uhler) = 0.04
Pseudo-R² (McFadden) = 0.02
AIC = 1732.56, BIC = 1743.13 
Df = 1454

## Model 2
```{r}

rq1data <- rq1data %>% mutate(squared = p_score^2)

sqEq <- "viewed~p_score + squared"
rq1mod2 <- glm(sqEq, rq1data, family = "binomial")
...
```
Significant quadratic relationship -- slope of -.06 log odds which for each unit of perforance increase is  .94 times as likely to view feedback. But there is an acceleration to the change of 0.0006489 log odds which is 1.00065 in odds. This means for each unit increase in performance the rate of change increases by that much. This seems very small increase but it is significant at the .001 level. 
...
AIC = 1717.99, BIC = 1733.84 
Df = 1453

Fits significantly better difference in X-square 16.577  p < .001 

## Model 3
```{r}

rq1data <- rq1data %>% mutate(cubed = p_score^3)

cubeEq <- "viewed~p_score + squared + cubed"
rq1mod3 <- glm(cubeEq, rq1data, family = "binomial")
...
No significant cubic relationship 

MODEL FIT:
χ²(3) = 61.07, p = 0.00
Pseudo-R² (Cragg-Uhler) = 0.06
Pseudo-R² (McFadden) = 0.03
AIC = 1717.56, BIC = 1738.69 
Df = 153
...
plot <-  ggplot(rq1data) +theme (axis.text.x = element_text(face = "bold", color = colors[4], 
                             size =15),
...
rq1Linplot <- plot + geom_point(aes(x = p_score, y = viewed), position = "jitter") + geom_smooth(aes(x = p_score, y = linprobs), method = "glm", 
    method.args = list(family = "binomial"), 
    se = FALSE) 

rq1Linplot

rq1Sqplot <- plot + geom_point(aes(x = p_score, y = viewed), position = "jitter") + geom_smooth(aes(x = p_score, y = sqprobs), method = "glm", method.args = list(family = "binomial"),  formula = y~ x + poly(x,2)) + scale_x_continuous(name = "Performance on Exam") + scale_y_continuous(name = "Probability of Viewing Feedback")
rq1Sqplot
...
rq1Cubeplot <- plot + geom_point(aes(x = p_score, y = viewed), position = "jitter") + geom_smooth(aes(x = p_score, y = cubeprobs), method = "glm", method.args = list(family = "binomial"),  formula = y~ x + poly(x, 2) + poly(x,3)) 
...
performance_feedback <- opt_out_data %>%  group_by(viewed) %>% summarise(variable = "Subsequent Score", n = n(), 
                                                    mean = mean(f_score, na.rm=T), 
                                                    sd = sd(f_score, na.rm = T), 
                                                    median = median(f_score, na.rm = T), 
                                                    MAD = mad(f_score, na.rm = T), 
...
eq1 <- f_score ~ viewed + (1|student_id)
...
```
Yes, when accounting for with-in individual variances, students who viewed their feedback scored higher(M= 79.44, SD = 14.55)  than those who did not (M = 70.81, SD = 14.56), t = 4.94 p < .01 
...
### Performance over time by viewed  
```{r}
eq3 <- f_score ~ time + viewed + time*viewed + (1|student_id)
