...
o = sum2$coefficients[1] + (sum2$coefficients[2]*x) + (sum2$coefficients[3]*x^2)
...
performance_feedback <- data %>% group_by(time) %>%  group_by(viewed, .add = T) %>% summarise(variable = "Subsequent Score", n = n(), 
                                                    mean = mean(f_score, na.rm=T), 
                                                    sd = sd(f_score, na.rm = T), 
                                                    median = median(f_score, na.rm = T), 
                                                    MAD = mad(f_score, na.rm = T), 
...
write.csv(performance_feedback, file = "overall feedback viewing over time.csv")
