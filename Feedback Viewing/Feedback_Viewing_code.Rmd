...
mstudent <- mdata %>% filter(!is.na(pRep)) %>% 
  group_by(student_id, time, attempt ) %>% 
  summarise(
    n = sum(numRep, na.rm = T),
    d = sum(pastMistakes, na.rm = T),
    pRep = n/d + 100
  ) 
...
(sum(nstudent$n)/sum(nstudent$d))/369

nmstudent <- nstudent%>% filter(n>0)

(sum(nmstudent$n)/sum(nmstudent$d))/314

314/369

mTime <- mstudent %>% group_by(time) %>% 
...
mAttempt <- mstudent %>% group_by(attempt) %>% 
  summarise( time = "none",
     n = n(),
    num = sum(n), 
    d = sum(d),
    mean = num/d
  ) %>% select(time, attempt, n,num, d, mean)

mQ <- mstudent %>% group_by(time, attempt) %>% 
  summarise(
    n = n(),
    num = sum(n), 
    d = sum(d),
...
write.csv(mdesc, "mistakeDescriptives.csv")
```
...
rq4eq1 <- "pRep~attempt + (1|student_id) + (1|attempt)"
...
pRep ~ time + viewed + time\*viewed + optin + optin\*viewed + optin\*time + optin\*viewed\*time

%rep = b0*time1 + b1*time2 + b2*time3 + b3*time4 + b4*Viewed*time1 + b5*Viewed*time2 + b6*Viewed*time3 + b7*Viewed*time4 + b8*Time1*optin + b9*Time2*optin + b10*Time3*optin + b11*Time4*optin   + b12*Time1*optin*Viewed  + b13*Time2*optin*Viewed  + b14*Time3*optin*Viewed  + b15*Time4*optin*Viewed 
