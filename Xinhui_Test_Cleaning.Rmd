...
## write the file
```{r}
write.csv(exam1_full, file = "exam1_writtenQs.csv")
```
...
for (file in Exam2) {
  ...
  # Select the required columns
  exam2 <- exam2 %>% 
    select("anon_id", "View.Count", "Status", contains("floater"), contains("Construct.Validity") )
  
  # Rename the columns
  q_names <- c("anon_id", "views", "Status", "floater", "cv")
...
# Combine all results into a single data frame
final_result <- bind_rows(results_list2, .id = "source")
