...
```{r}
write.csv(full_data, "2023_initial_dataset.csv", row.names = F)
```
...
```{r}
final_data <- left_join(full_data, exam1_end, by = "anon_id" )
```
...
```{r}
final_data <- left_join(full_data, exam2_end, by = "anon_id" )
```
...
```{r}
final_data <- left_join(full_data, exam3_end, by = "anon_id" )
```
...
```{r}
final_data <- left_join(full_data, exam4_end, by = "anon_id" )
```
...
```{r}
final_data <- left_join(full_data, exam5_end, by = "anon_id" )
```

```{r}
write.csv(full_data, "2023_cleaned.csv", row.names = F)
```
