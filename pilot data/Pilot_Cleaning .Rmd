...
```{r}
Final_data <- left_join(dem_data, canvas_final, by = SID)
```
...
for (file in Unit1) {
  unit1 <- read.csv(file)
   unit1 <- unit1 %>% select("SID","View.Count", "Status", 14, 16, 17, 18, 19, 22, 23, 28, 29, 30, 31)
   q_names <- c("SID", "views1", "Status", "h0", "h1", "h2", "pv_iv", "pv_op", "ov_dv", "ov_op", "claim_ex", "temp", "alt_ex", "interpret")
...
for (file in Unit2) {
  unit2 <- read.csv(file)
   ...
   q_names <- c("SID", "views2", "Status", "pv_op", "pv_cv", "ov_op", "cv_ov", "effect", "ov_reliability", "h1", "distress_ex")
...
```{r}
unit1_calc <- unit1_short %>% select(4:14)
...
full_data <- left_join(Final_data, unit2_end, by = "SID" )
```
...
full_data <- left_join(Final_data, unit3_end, by = "SID" )
```
...
full_data <- left_join(Final_data, unit4_end, by = "SID" )
```
...
full_data <- left_join(Final_data, unit5_end, by = "SID" )
```